...
makeLuaText('text', 'PRESS SPACE TO SKIP SONG', 1520, -120, 535)
setTextSize('text', 30)
end
...
function onCreatePost()
	...
	setPropertyFromClass('GameOverSubstate', 'loopSoundName', 'gameOverEmpty'); --put in mods/music/
	setPropertyFromClass('GameOverSubstate', 'endSoundName', 'gameOverEndEmpty'); --put in mods/music
noteTweenX('lee', 4, 412, 0.1, 'smootherStepOut')
	noteTweenX('dff', 5, 524, 0.1, 'smootherStepOut')
	noteTweenX('uvv', 6, 636, 0.1, 'smootherStepOut')
	noteTweenX('rzz', 7, 748, 0.1, 'smootherStepOut')
...
if curStep < 272 and keyJustPressed('space') then
loadSong('unhinged','hard')
end
end
...
stepHitFuncs = { --a bunch of timed events, timed to steps
	...
	[1296] = function()
	doTweenAlpha('sanslolback', 'defeat', 100, 0.00000000000001, 'smootherInOut')
	addLuaSprite('vignette', true);
	removeLuaSprite('shine', false);
		   makeLuaSprite('flash', '', 0, 0);
        makeGraphic('flash',1980,1020,'ffffff')
	      addLuaSprite('flash', true);
	      setLuaSpriteScrollFactor('flash',0,0)
	      setProperty('flash.scale.x',2)
	      setProperty('flash.scale.y',2)
	...
	triggerEvent('Change Character', 1, 'blacknormal');
		...
		[1551] = function()
 doTweenZoom('zommingsuspenseagain', 'camGame', 1.1, 3, 'quadInOut')
	end,
	
	[1567] = function()
	cancelTween('zommingsuspenseagain')
	doTweenAlpha('sanslolgoneagain', 'defeat', 0, 0.00000000000001, 'smootherInOut')
	addLuaSprite('vignette', true);
	addLuaSprite('shine', true);
		   makeLuaSprite('flash', '', 0, 0);
        makeGraphic('flash',1980,1020,'ff0000')
